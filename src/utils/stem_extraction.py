...
from src.data_processing.transcription import StemTranscriber
...
class StemExtractor:
    ...
    def extract_stems_from_file(self, audio_path: str, output_dir: Optional[str] = None,
        ...
        # Verify that the audio file exists and print its details
        print(f"Audio file path: {audio_path}")
        print(f"Audio file exists: {os.path.exists(audio_path)}")
        print(f"Audio file is a file: {os.path.isfile(audio_path)}")
        print(f"Audio file size: {os.path.getsize(audio_path) if os.path.exists(audio_path) else 'N/A'}")
        
        # Load the model in inference mode
        with torch.no_grad():
            # Call demucs directly as a subprocess
            import subprocess
            
            # Construct the demucs command
            demucs_cmd = ["demucs", "-v", "-n", self.model_name, "-o", output_dir]
            
            if len(self.source_names) == 2:
                demucs_cmd.append("--two-stems=vocals")
        ...
        print(f"Looking for stems in: {stem_dir}")
        
        for source_name in self.source_names:
            stem_path = os.path.join(stem_dir, f"{source_name}.wav")
            print(f"Checking for stem at: {stem_path}")
