...
class DrumGenerator(Generator):
    ...
    def generate(self, 
                ...
                bpm: int = 120) -> Optional[np.ndarray]:
        """
        Generate drum content based on a text prompt.
        
        Args:
            prompt: Text description of the drums to generate
        ...
        """
        # Enhance the prompt with drum-specific terms and BPM
        enhanced_prompt = f"Drum beat at {bpm} BPM: {prompt}, percussion, rhythm section, drums only"
        
        # Call the parent generate method with enhanced prompt
    ...
    def _enhance_drum_prompt(self, prompt: str, genre: str = "") -> str:
        """
        Enhance a prompt with drum-specific terms.
        ...
        """
        drum_terms = {
            "trap": "trap drums, 808 drums, hi-hats, snare rolls",
            "rock": "rock drums, kick drum, snare, cymbals, powerful",
            "jazz": "jazz drums, brushes, swing, subtle",
            "electronic": "electronic drums, programmed, synthetic",
            "hip-hop": "hip-hop drums, boom bap, heavy kick, crisp snare"
        }
        
        genre_terms = drum_terms.get(genre.lower(), "drums, percussion, rhythm")
        return f"{prompt}, {genre_terms}, drums only"
